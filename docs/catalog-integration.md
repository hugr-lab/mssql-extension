...
`MSSQLCatalog::GetDefaultSchema()` names the schema that unqualified names resolve against. That covers:

- a two-part name, `db.table`;
- `USE db`;
- CTAS into `db.table`;
- COPY into `db.table` or `mssql://db/table`.

It is `dbo` unless the `default_schema` option says otherwise (issue #322):

| Source | Key |
|--------|-----|
| MSSQL Secret | `default_schema` |
| ADO.NET Connection String | `DefaultSchema` (also `Default Schema`, `default_schema`) |
| URI Query Parameters | `default_schema` |
| ATTACH Options | `default_schema` (overrides the others) |

Some rules for the value:
...
1. **The schema list** — `EnsureSchemasLoaded`, the same light `sys.schemas` query every catalog lookup starts with, and a no-op when the list is already loaded. It comes first on both paths (issue #376), under the same lock as the load that follows, so an invalidation cannot land in between. Without it, the per-schema path marked the list loaded while holding only the one schema it was given, so every other schema stopped existing for the session. The whole-catalog path left the list unloaded, so the first catalog access afterwards cleared the cache and loaded the whole catalog a second time.
2. **The metadata**:
   ...
   - With a schema, one query covers that schema's tables and columns (`BULK_METADATA_SCHEMA_SQL_TEMPLATE`). The name matches exactly, or ignoring case when exactly one schema matches that way. A schema that is not in the list, because it is absent from the database or hidden by `schema_filter`, is refused by name and gets no entry.
