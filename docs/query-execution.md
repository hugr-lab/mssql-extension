...
Each parameter is declared from the **column** it is compared with, never narrower than the constant. A string constant keeps the column's kind — `varchar` for a `varchar` column, which is what keeps an index on it seekable — with one rule for non-ASCII text (issue #361): the constant travels as `varchar` when both the column's and the database's code page can hold every character of it (a `varchar` parameter takes the *database's* page and the comparison converts it to the column's), and as `nvarchar` otherwise. The pages are the server's own answer, `COLLATIONPROPERTY(collation, 'CodePage')`, read with the column metadata and the database collation; `mssql::CodePageCanEncode` answers from tables for the single-byte pages 874 and 1250–1258, and a double-byte or unknown page keeps the `nvarchar` form. `IN` lists and `IS NULL` stay literal.
