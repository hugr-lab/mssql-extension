...
void hexdump(const uint8_t* data, size_t len, const char* prefix = "", size_t max_len = 100) {
    ...
    if (len > max_len) printf("... (%zu more bytes)", len - max_len);
    printf("\n");
}
...
std::vector<uint8_t> build_prelogin(bool encrypt = false) {
    ...
    auto header = build_tds_header(TDS_TYPE_PRELOGIN, TDS_STATUS_EOM,
                                   TDS_HEADER_SIZE + payload.size(), 0);  // Default, caller should set
...
std::vector<uint8_t> build_login7(const std::string& host, const std::string& user,
    ...
    std::vector<uint8_t> packet;
    auto header = build_tds_header(TDS_TYPE_LOGIN7, TDS_STATUS_EOM,
                                   TDS_HEADER_SIZE + payload.size(), 0);
    packet.insert(packet.end(), header.begin(), header.end());
    packet.insert(packet.end(), payload.begin(), payload.end());
    return packet;
}
...
std::vector<uint8_t> build_all_headers() {
    ...
    for (int i = 0; i < 8; i++) headers.push_back(0);
...
std::vector<std::vector<uint8_t>> build_sql_batch_packets(const std::string& sql,
                                                          size_t max_packet_size,
                                                          uint8_t& start_pkt_id,
                                                          bool include_all_headers = true) {
...
int main(int argc, char* argv[]) {
    ...
    if (argc > 1) sql_length = atoi(argv[1]);
        ...
        if (fd >= 0 && connect(fd, (struct sockaddr*)&addr, sizeof(addr)) == 0) {
            ...
            if (!response.empty()) {
                ...
                printf("    SQL length: %zu bytes (%zu UTF-16LE, no headers)\n",
                       multi_sql.length(), multi_sql.length() * 2);
                printf("    Packet count: %zu\n", packets.size());
