...
// My debug version
std::vector<uint8_t> debug_build_login7(const std::string& host, const std::string& user,
...
int main() {
    ...
    for (int i = 36; i < 90; i += 4) {
        ...
        if (idx == 9) {
            std::cout << "  ClientID: ";
            for (int j = 0; j < 6; j++) std::cout << std::hex << std::setw(2) << std::setfill('0') << (int)payload[72+j] << " ";
            std::cout << "\n";
