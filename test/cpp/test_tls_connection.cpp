...
void test_tls_cipher_and_version(const TestConfig& config) {
    ...
    // TLSv1.2 or TLSv1.3
    assert(version.find("TLSv1.2") != std::string::npos ||
           version.find("TLSv1.3") != std::string::npos);

    // Verify we got a strong cipher (contains AES or CHACHA20)
    assert(cipher.find("AES") != std::string::npos ||
...
void test_query_over_tls(const TestConfig& config) {
    ...
    while (true) {
        ssize_t received = conn->ReceiveData(buffer.data() + total_received,
                                              buffer.size() - total_received, 5000);
        if (received <= 0) break;
...
void test_tls_connection_pool(const TestConfig& config) {
    ...
    std::cout << "Pool stats: created=" << stats.connections_created
              << ", acquires=" << stats.acquire_count << std::endl;
...
void test_tls_vs_plaintext(const TestConfig& config) {
    ...
    std::cout << "TLS connection: TLS=" << (tls_conn->IsTlsEnabled() ? "yes" : "no")
              << ", SPID=" << tls_conn->GetSpid() << std::endl;
...
void test_tls_connection_timing(const TestConfig& config) {
    ...
    auto tcp_time = std::chrono::duration_cast<std::chrono::milliseconds>(
        std::chrono::steady_clock::now() - start).count();
    ...
    auto auth_time = std::chrono::duration_cast<std::chrono::milliseconds>(
        std::chrono::steady_clock::now() - start).count();
...
void test_large_query_over_tls(const TestConfig& config) {
    ...
    while (true) {
        ...
        auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(
            std::chrono::steady_clock::now() - start).count();
        if (elapsed > 30000) break;  // 30 second max
    }
