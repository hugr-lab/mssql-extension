...
std::string BytesToHex(const duckdb::vector<uint8_t> &bytes) {
	std::stringstream ss;
...
void test_encode_int8() {
	...
	duckdb::vector<uint8_t> expected = {0x01, 0x2A};
...
void test_encode_int16() {
	...
	duckdb::vector<uint8_t> expected = {0x02, 0xE8, 0x03};
	ASSERT_BYTES_EQ(buffer, expected);
...
void test_encode_int32() {
	...
	duckdb::vector<uint8_t> expected = {0x04, 0x40, 0xE2, 0x01, 0x00};
	ASSERT_BYTES_EQ(buffer, expected);
...
void test_encode_int64() {
	...
	duckdb::vector<uint8_t> expected = {0x08, 0xCB, 0x04, 0xFB, 0x71, 0x1F, 0x01, 0x00, 0x00};
	ASSERT_BYTES_EQ(buffer, expected);
...
void test_encode_uint8() {
	...
	duckdb::vector<uint8_t> expected = {0x01, 0xFF};
	ASSERT_BYTES_EQ(buffer, expected);
...
void test_encode_bit() {
	...
	duckdb::vector<uint8_t> expected = {0x01, 0x01};
	ASSERT_BYTES_EQ(buffer, expected);
...
void test_encode_float() {
	...
	duckdb::vector<uint8_t> expected = {0x04, 0x00, 0x00, 0x00, 0x00};
	ASSERT_BYTES_EQ(buffer, expected);
...
void test_encode_decimal() {
	...
	duckdb::vector<uint8_t> expected = {0x05, 0x01, 0x39, 0x30, 0x00, 0x00};
	ASSERT_BYTES_EQ(buffer, expected);
...
void test_encode_binary() {
	...
	duckdb::vector<uint8_t> expected = {0x04, 0x00, 0x01, 0x02, 0x03, 0x04};
	ASSERT_BYTES_EQ(buffer, expected);
...
void test_encode_guid() {
	...
	duckdb::vector<uint8_t> expected = {0x10, 0x00, 0x84, 0x0E, 0x55, 0x9B, 0xE2, 0xD4, 0x41,
...
void test_encode_null() {
	...
	duckdb::vector<uint8_t> expected = {0x00};
	ASSERT_BYTES_EQ(buffer, expected);
...
void test_encode_multiple_values() {
	...
	// Encode: int32(42), bool(true), binary("hi"), NULL (fixed)
	BCPRowEncoder::EncodeInt32(buffer, 42);
	BCPRowEncoder::EncodeBit(buffer, true);
	BCPRowEncoder::EncodeBinary(buffer, string_t("hi"));
	BCPRowEncoder::EncodeNullFixed(buffer);

	// Expected sequence:
	// Int32: [04] [2A 00 00 00]
	// Bit:   [01] [01]
	// BIN:   [02 00] [68 69] ("hi" as varbinary: 2-byte length, raw bytes)
	// NULL:  [00]
	duckdb::vector<uint8_t> expected = {
		0x04, 0x2A, 0x00, 0x00, 0x00,  // int32(42)
		0x01, 0x01,					   // bit(true)
		0x02, 0x00, 0x68, 0x69,		   // binary("hi")
		0x00						   // NULL
	};
	ASSERT_BYTES_EQ(buffer, expected);
