...
# Stock CLI must LOAD the community extension; the static candidate must not.
LOAD_LINE=""
...
case "$VARIANT" in
  ...
  w-plain0)   # baseline form: no settings that 0.2.2 lacks
    STMT="COPY (FROM $SRC) TO 'db.dbo.WB' (FORMAT bcp, CREATE_TABLE true, REPLACE true);";;
  w-maxlen)
...
rm -f "$SCRATCH/r_export_out.parquet"
