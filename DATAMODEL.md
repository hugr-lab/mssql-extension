...
- **The rowid key is chosen, not assumed** (spec 077 W1). The discovery statement returns every unique index on the table — the primary key is one of them — filtering in SQL on nothing but `is_unique`; `ChooseRowIdKey` (`catalog/mssql_rowid_key_choice.hpp`, header-only, unit-tested) takes the primary key if usable, else a usable unique index in a documented order (single-column identity key, fewest key columns, narrowest, lowest `index_id`). Usable: not filtered, not disabled, every key column NOT NULL, and no key column whose rowid literal cannot match it — the lossy-read types (`sql_variant`, `hierarchyid`, #354) and `datetime`/`smalldatetime` (#358). An unusable primary key falls through; a rejected candidate is kept with its reason and the refusal names it. Cost measured server-side: cheaper than the old primary-key-only query on the test database (38 vs 151 µs — two joins gone), 6 µs (14%) more than a join-less primary-key-only form on a 3000-table catalog.
