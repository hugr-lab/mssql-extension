...
static void MSSQLPreloadCatalogExecute(DataChunk &args, ExpressionState &state, Vector &result) {
	...
	UnaryExecutor::Execute<string_t, string_t>(
		catalog_names, result, args.size(), [&](string_t catalog_str) -> string_t {
			...
			// Refused while this catalog has a server transaction open (issue
			// #380; scoped to the catalog in the review of #382) -- see
			// mssql_refresh_cache.
			if (catalog.HasOpenServerTransaction(client_context)) {
				throw InvalidInputException(
					"mssql_preload_catalog cannot run inside a transaction that has used catalog '%s': it would "
					"load the catalog's metadata while the transaction may hold uncommitted changes. Use "
