...
| `mssql_acquire_timeout` | 30 | How long `Acquire` waits for a pooled connection to be **released** when the pool is at its limit. It is **not** how long a failed creation takes to report: since issue #302 a factory failure (expired Azure AD token, wrong password, refused dial) with nothing active fails at once and names the reason — `could not create a connection: Login failed for user 'sa'.` — and with others active the pool keeps waiting for a release but retries creation on a backoff (250 ms doubling to 4 s), not on every wakeup. Before, a failed creation was treated as a full pool and reported as `(timeout)` after this many seconds, with the reason discarded. |
