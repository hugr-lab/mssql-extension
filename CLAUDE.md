...
- **ORDER BY / TOP pushdown** (`MSSQLOptimizer`, opt-in `mssql_order_pushdown`): removes DuckDB's sort, so a key is pushed only when `MSSQLColumnInfo::OrdersLikeDuckDB()` holds — an allow-list (numeric, bit, date/time types, not `datetime2(7)`, which the codec reads as TIMESTAMP_NS and NULLs past 2262; no string, no nvarchar, binary, uniqueidentifier, json, sql_variant; #362) — or, under a LIMIT only, `OrdersLikeDuckDBAsBytes()`: a UTF-8 `varchar`/`char` ordered as `CAST(col AS varbinary(max))` (as text the server pads with spaces: `ab`+TAB before `ab`; as bytes the only residual is a trailing NUL tie; not sargable, hence LIMIT only). A string-valued function key, and a date part of a `datetimeoffset`, are never pushed. Under a LIMIT a partial key list pushes nothing. A nullable key whose NULL placement differs from the server's (NULL lowest) is pushed only under a LIMIT, with a leading `CASE WHEN k IS NULL …` key; a plain ORDER BY keeps DuckDB's sort. TOP N is not pushed while any non-optional filter stays client-side (the server would take N rows before it). Spec 079's remote-pushdown writer is to use the same predicate (D4). Every identifier the extension sends goes through `mssql::QuoteIdentifier` (`query/mssql_identifier.hpp`, self-contained); a name inside a string literal through `mssql::NVarcharLiteral` (`query/mssql_sql_params.hpp`).
