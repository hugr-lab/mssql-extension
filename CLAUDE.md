...
| `mssql_refresh_cache(context)` | Scalar | Refresh metadata cache (eager full reload). Refused inside a transaction (issue #380) |
...
| `mssql_preload_catalog(context [, schema])` | Scalar | Bulk-load all metadata in one round trip. Refused inside a transaction (issue #380) |
