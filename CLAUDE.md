...
- **ORDER BY / TOP pushdown** (`MSSQLOptimizer`, opt-in `mssql_order_pushdown`): removes DuckDB's sort, so a key is pushed only when `MSSQLColumnInfo::OrdersLikeDuckDB()` holds — strings only under `_BIN2` (nvarchar) / `_BIN2_UTF8` (varchar), never uniqueidentifier / sql_variant (#362; there was no collation check at all before). A nullable key whose NULL placement differs from the server's (NULL lowest) gets a leading `CASE WHEN k IS NULL …` key instead of stopping the pushdown. The same predicate is the remote-pushdown writer's (spec 079 D4). Every identifier the extension sends goes through `mssql::QuoteIdentifier` (`query/mssql_sql_params.hpp`); a name inside a string literal through `mssql::NVarcharLiteral`.
