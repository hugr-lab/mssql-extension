...
- **ORDER BY / TOP pushdown** (`MSSQLOptimizer`, opt-in `mssql_order_pushdown`): removes DuckDB's sort, so a key is pushed only when `MSSQLColumnInfo::OrdersLikeDuckDB()` holds — an allow-list (numeric, bit, date/time types; the one string is `varchar`/`char` under `_BIN2` + UTF-8; never nvarchar, which diverges above the BMP, nor binary / uniqueidentifier / json / sql_variant; #362). A string-valued function key is never pushed. A nullable key whose NULL placement differs from the server's (NULL lowest) is pushed only under a LIMIT, with a leading `CASE WHEN k IS NULL …` key; a plain ORDER BY keeps DuckDB's sort. TOP N is not pushed while any non-optional filter stays client-side (the server would take N rows before it). The same predicate is the remote-pushdown writer's (spec 079 D4). Every identifier the extension sends goes through `mssql::QuoteIdentifier` (`query/mssql_identifier.hpp`, self-contained); a name inside a string literal through `mssql::NVarcharLiteral` (`query/mssql_sql_params.hpp`).
