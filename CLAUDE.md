...
- **ORDER BY / TOP pushdown** (`MSSQLOptimizer`, opt-in `mssql_order_pushdown`): removes DuckDB's sort, so a key is pushed only when `MSSQLColumnInfo::OrdersLikeDuckDB()` holds — an allow-list (numeric, bit, date/time types, not `datetime2(7)`, which the codec reads as TIMESTAMP_NS and NULLs past 2262; no string, no nvarchar, binary, uniqueidentifier, json, sql_variant; #362) — or, under a LIMIT only, `OrdersLikeDuckDBAsBytes()`: a bounded UTF-8 `varchar` (not `char(n)`, stored padded and read trimmed; not MAX) ordered as `CAST(col AS varbinary(n))` (as text the server pads with spaces: `ab`+TAB before `ab`; as bytes the only residual is a trailing NUL tie; not sargable, hence LIMIT only). A string-valued function key, and a date part of a `datetimeoffset`, are never pushed. A partial key list pushes nothing (the DuckDB sort stays anyway). Filter pushdown maps no string function (`lower`/`upper`/`trim`/`length` run client-side: the server's case mapping is not DuckDB's, and a pushed `upper(x) = 'ẞ'` lost rows) and no date part of a TIMESTAMP WITH TIME ZONE. A nullable key whose NULL placement differs from the server's (NULL lowest) is pushed only under a LIMIT, with a leading `CASE WHEN k IS NULL …` key; a plain ORDER BY keeps DuckDB's sort. TOP N is not pushed while any non-optional filter stays client-side (the server would take N rows before it). Spec 079's remote-pushdown writer is to use the same predicate (D4). Every identifier the extension sends goes through `mssql::QuoteIdentifier` (`query/mssql_identifier.hpp`, self-contained); a name inside a string literal through `mssql::NVarcharLiteral` (`query/mssql_sql_params.hpp`).
...
| `mssql_catalog_native_types` | true | Report `MSSQL_VARCHAR(n)` / `MSSQL_NVARCHAR(n)` for the string columns of attached tables instead of a bare `VARCHAR` (spec 060). Since spec 079 PR A also for `mssql_scan` / `mssql_scan_params`, through the same `NativeDuckDBType`, so `CREATE TABLE … AS SELECT * FROM mssql_scan(…)` keeps the source's lengths and collations. There a `char`/`varchar` is annotated only under a UTF-8 collation: a raw scan hands a code-page varchar's bytes over untranscoded (#224), so it stays plain VARCHAR. `prepared := true` reports the same types: sp_prepare's COLMETADATA carries a collation id rather than a name, so when a column's collation is UTF-8 (the COLMETADATA bit, set whether it travelled as varchar or, without UTF8SUPPORT, as nvarchar) the bind also asks `sp_describe_first_result_set` (one more round trip), takes its type per column while the names agree (stopping at the first disagreement), and leaves an unconfirmed UTF-8 column plain VARCHAR (review of #387). A statement run at bind is typed from COLMETADATA the same way (nvarchar annotated, varchar plain). The execution-time shape check compares string lengths, not kinds: the bind names the declared type, the stream carries the wire type. A statement the server cannot describe, which therefore runs at bind, stays plain VARCHAR. This is what lets a target created from an MSSQL source inherit its declared lengths and collations with no cast written by anyone. Costs a changed `DESCRIBE` / `duckdb_columns()` type name, which is what turning it off restores. Read when a table entry is built, so `mssql_invalidate_cache()` applies a change to already-loaded tables. |
