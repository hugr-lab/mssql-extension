...
a core mechanism; the T-SQL writer that mechanism needs is the same work D2
would have done twice. What survives: D3's token-loop consolidation, D5's dead
setting, and the tests. Kept as written for the record.
**Date:** 2026-08-06
**Depends on:** nothing new (the 3-arg plan hooks exist in the pinned DuckDB —
re-verified on `v2.0-cyanoptera`: `catalog.hpp:365-370`, called by
`plan_update.cpp:30` / `plan_delete.cpp:41`)
