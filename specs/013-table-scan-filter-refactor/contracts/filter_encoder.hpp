...
// - Types in duckdb::mssql do NOT use MSSQL prefix (e.g., FilterEncoder, not MSSQLFilterEncoder)
 ...
 * @param function_name DuckDB function name (will be lowercased)
 * @return FunctionMapping if supported, nullopt if not
 */
std::optional<FunctionMapping> GetFunctionMapping(const std::string& function_name);
...
struct ExpressionEncodeContext {
    const std::vector<idx_t>& column_ids;           // Projection mapping
    ...
    ExpressionEncodeContext(
        const std::vector<idx_t>& col_ids,
        const std::vector<std::string>& col_names
    ) : column_ids(col_ids), column_names(col_names), depth(0) {}

    // Create child context with incremented depth
    ExpressionEncodeContext child() const {
        ExpressionEncodeContext ctx(column_ids, column_names);
        ctx.depth = depth + 1;
        return ctx;
    }
...
private:
     ...
     */
    static std::optional<std::string> GetComparisonOperator(ExpressionType type);

    /**
     * Get T-SQL arithmetic operator for DuckDB ExpressionType.
     */
    static std::optional<std::string> GetArithmeticOperator(ExpressionType type);
};
