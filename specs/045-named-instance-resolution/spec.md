...
- **FR-003**: The resolver MUST send a `CLNT_UCAST_INST` packet (`0x04` + MBCS instance name + `0x00`) to `host:1434/udp` as specified in [MC-SQLR] §2.2.3. Note: while [MC-SQLR] §2.2 defines all text strings as MBCS, SQL Server's own instance-name grammar restricts instance names to `[A-Za-z0-9_$#]{1,16}` (ASCII), so the bytes on the wire are always ASCII in practice.
- **FR-004**: The resolver MUST parse the `SVR_RESP` body — a NUL-terminated `key;value;key;value;…` MBCS string with one record per instance — and extract the `tcp;<port>` token for the matching `InstanceName`. Comparison is case-insensitive (ASCII case-fold, which is correct for the documented instance-name grammar).
