...
- [ ] T018 [US1] Implement `codec::integer::FormatDdlTypeName` in `src/codec/integer_codec.cpp` — returns `"TINYINT"`/`"SMALLINT"`/`"INT"`/`"BIGINT"`/`"DECIMAL(20,0)"` (UBIGINT)/`"DECIMAL(38,0)"` (HUGEINT) — **identical in both `DdlContext` values per FR-025/FR-028**. HUGEINT path forwards to `codec::decimal::FormatDdlTypeName` (precision=38, scale=0). UHUGEINT (if it routes through Integer family) likewise maps to `"DECIMAL(38,0)"`. The `(void)ctx;` line documents the ignored parameter. Spec changed in post-Phase-2 user feedback (was: throw for HUGEINT in CtasCreateTable).
